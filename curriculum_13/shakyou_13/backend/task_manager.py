...
main.py（受付・HTTPの世界）と models.py（DBの世界）の「間」に立つ層。
main.pyはHTTPステータスコード（404, 403など）を知っているが、
DB操作の詳細は知らない。逆にこのファイルはHTTPのことを一切知らず、
「None・forbidden・成功したTaskオブジェクト」という
Python的な戻り値だけを返す。この責任分担により、
「タスクが見つからない」という同じロジックを、
Web API以外（例：バッチ処理）からも再利用できるようになる。
"""
