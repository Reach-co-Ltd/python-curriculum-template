...
リクエストが来る → main.py（受付） → task_manager.py（処理） → models.py（DBの形）
                                          ↑
                                    ここでDBとの「接続」そのものを用意する

このファイルが用意した get_db() を、main.py の各エンドポイントが
Depends(get_db) という形で呼び出すことで、
「1回のリクエストにつき1つのDBセッション」が自動的に用意される。
"""
